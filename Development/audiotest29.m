...
while (GetSecs - startSecs < t) % the t sets total time allotted per wav file
    ...
    if (keyIsDown && strcmp(press, 'no'))
        ...
        if strcmpi(KbName(keyCode),'t') % need close to t timing condition here
            break;
        end       
    end
end
...
disp('White Noise');disp(block_num);
...
play(player);
[secs, press] = KbTime(30); % does this need to wait for 30 secs? YES!
