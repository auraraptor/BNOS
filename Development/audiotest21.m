...
try % This is for fullscreen display
    ...
    Screen('Flip',w);
	synchKey = KbName('t'); % synch with t 
    while 1
...
timingKey = KbName('t'); % use t character to sync with mri
secs =''; % initialize
press = 'no';
startSecs = GetSecs;
while (GetSecs - startSecs < t) % the t sets total time allotted per wav file
    ...
    if (keyIsDown && strcmp(press, 'no'))
        ...
        press = 'yes';
        if keyCode(timingKey) % need close to t timing condition here
            break;
        end       
    end
end
